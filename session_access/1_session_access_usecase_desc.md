...
- **Chia sẻ Session với User** → đề xuất UC chia sẻ Session
...
- UC "Xem chi tiết Session" có các extension points:
...
- UC "Yêu cầu truy cập Session", "Hủy yêu cầu truy cập Session" có thể thực hiện khi **chưa có quyền truy cập** hoặc quyền bị hạn chế
- UC "Cập nhật quyền realtime" hoạt động thông qua WebSocket connection tự động khi User vào Session
...
| **Chia sẻ Session với User** | UC này cho phép User thêm User cụ thể vào Session với mức quyền Reader, Contributor hoặc Manager. |
...
| **Yêu cầu truy cập Session** | UC này cho phép User gửi yêu cầu để được cấp quyền truy cập vào Session. |
| **Hủy yêu cầu truy cập Session** | UC này cho phép User hủy yêu cầu truy cập đang chờ xử lý. |
